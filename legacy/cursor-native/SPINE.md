> **SUPERSEDED — archive only.** This is the Cursor-era copy, kept for history. The
> living canon is **SPINE v2.0 (2026-08-22)** at the repo root; do not run the method
> from this file. Its version line predates the re-sync and is not comparable.

# SPINE — the method engine (single owner, all tiers inherit)

**Version line (machine-readable):** `spine v1.2 (2026-07-24)`
...
REVIEW = **red 🔴**, and it TRAILS the reviewer's own model color (boss ruling 2026-08-09):
🔵🔴 = Codex reviewing · 🟢🔴 = Gemini reviewing · ⚫🔴 = Grok reviewing · 🟠🔴 = Claude reviewing.
Red alone is never enough: the vendor color has to stay visible, because the whole value of a review
is WHO ran it. 🔵🟣 building then 🔵🔴 reviewing its own work is the failure this notation exists to
make obvious at a glance.

BUILDING = **purple 🟣**, trailing the builder's model color the same way (boss ruling 2026-08-09):
🔵🟣 = Codex building · 🟠🟣 = Claude building · ⚫🟣 = Grok building · 🟢🟣 = Gemini building.

So a run reads as WHO + WHAT in two glyphs:
> 🟡 two lanes. 🔵🟣 Codex on the validator · 🟠🟣 Claude on the parser → ⚫🔴 Grok reviewing both.

The first color is always the SEAT (who), the second is always the ACT (🟣 build / 🔴 review).
Unspent circles left for future SEATS: ⚪ white, 🟤 brown.

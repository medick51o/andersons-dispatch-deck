...
## YOUR LENS — STRUCTURAL PRECISION
You have reviewed this document before and know its cross-references. Weight Q2 and Q3.
Hunt whole SUBSYSTEMS that could live on demand, not just sentences. For each, state exactly
what summary must stay in the trunk so an orchestrator still knows the rule EXISTS and when to
go read it. A pointer that does not tell you when to follow it is worse than the prose.

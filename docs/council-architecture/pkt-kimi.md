...
## YOUR LENS — THE ADVERSARY (steelman the opposite)

Your job is to argue AGAINST the current architecture as persuasively as you honestly can. Make the strongest possible case that stacked consumer subscriptions are the wrong answer for this shop and that pay-per-token or owned compute wins. Then - and only then - state honestly where your own argument is weakest. A steelman that cannot name its own flaw is propaganda.
